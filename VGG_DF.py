...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler=None, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), '/root/LLM/Model/best_VGG_model.pth')
...
def main():
    ...
    # 加载最佳模型
    model.load_state_dict(torch.load('/root/LLM/Model/best_VGG_model.pth'))
    
    # 在测试集上评估
    test_accuracy, cm = test_model(model, test_loader)
    
    # 保存模型
    torch.save(model.state_dict(), '/root/LLM/Model/final_VGG_model.pth')
    torch.save(model, '/root/LLM/Model/full_VGG_model.pth')  # 保存整个模型
